...
from mcb_scripts.workspace import (
    LOCAL_PYTHON,
    ROOT,
    SCRIPTS,
...
class Param:
    """One declared parameter of a workspace command, from its TOML header."""
...
@dataclass(frozen=True, slots=True)
class Command:
    """One executable workspace command resolved from the scripts tree."""
...
@dataclass(frozen=True, slots=True)
class AliasTarget:
    """The verb/WHAT pair an alias resolves to."""
...
class Registry:
    ...
    def alias_target(self, verb: str) -> AliasTarget | None:
        """Return the verb/WHAT an alias resolves to, or None."""
...
def main(argv: Sequence[str] | None = None) -> int:
    """Entry point: discover workspace commands and dispatch the verb."""
...
def run(command: Command) -> int:
    ...
    return subprocess.run(
        [_resolve_shell(), str(command.path)], cwd=cwd, env=env, check=False
    ).returncode
...
def run_python(command: Command, env: Mapping[str, str], cwd: Path) -> int:
    ...
    try:
        ...
        # The dispatched script runs in-process, so it must resolve the shared
        # library from the canonical source root, never from scripts/.
...
def discover() -> Registry:
    """Build the registry by scanning the workspace scripts tree."""
    registry = Registry()
...
def has_header(path: Path) -> bool:
    """Report whether a file declares a workspace-command header."""
    try:
        header_data(path)
    except RegistryError:
        return False
...
def load_command(path: Path, expected_verb: str) -> Command:
    """Parse one workspace command file into a Command."""
...
def normalize_toml_table(data: RawTomlValue, path: Path) -> TomlTable:
    """Normalize a parsed TOML table into the supported value shapes."""
...
def normalize_toml_value(value: RawTomlValue, path: Path) -> TomlValue:
    """Normalize one parsed TOML value into a supported shape."""
...
def parse_alias_spec(alias: str, command: Command) -> tuple[str, str]:
    """Split one alias declaration into its verb and WHAT parts."""
...
def require_env(name: str, usage: str | None = None) -> str:
    ...
    The optional ``usage`` string is the caller's contract line; it is reported
    on failure so the operator learns WHICH selector was missing and how to
    supply it, instead of receiving a bare exit code.
    """
    value = env_value(name)
    if value:
        return value
    logger.error(usage or f"missing required environment value: {name}")
    raise SystemExit(2)
